...
// do nothing
for(int i=0;i<nvalues;i++) kv->add(key,keybytes, (char *)&head[i],(int)sizeof(head[i]));
}
 
/* standard_merge state */
else if(state.compare("SM")==0){
kv->add(key,keybytes, (char *)&min,(int)sizeof(min));//emit forward edge
kv->add((char *)&min,(int)sizeof(min),key, keybytes);// emit backward edges
for(int i=0;i<nvalues;i++){//emit forward edges from min id to all edges
if(head[i]!=min){
...
kv->add(key,keybytes,(char *)&min,(int)sizeof(min));
kv->add((char *)&min,(int)sizeof(min),key, keybytes);
for(int i=0;i<nvalues;i++){
  if(head[i]!=min) kv->add((char *)&min,(int)sizeof(min),(char *)&head[i],(int)sizeof(head[i]));
 }
}
  //  kv->add(key,keybytes,(char *)&nvalues,sizeof(int));
}
